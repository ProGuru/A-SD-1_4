...
Set::Set(char): S('A' + cnt++), n(0), A(new List(0)) // генератор множества. S('A' + cnt++) - устанавливает тег в виде первой буквы латинского алфавита для первого объекта класса, для 2-го объекта класса S = B, для 3-го S = C
{
	...
	if (rand()%p>q) Massiv[ n++ ] = i + 'A'; // ИЗМЕНИЛ АЛГОРИТМ "rand() % 2". в результате геренеции множества образуется массив символов, а также ведётся подсчёт мощности множества
    Massiv[n] = 0;
...
{  
    char dst(A->data), src(B.A->data);	//Инициализация адресов/счётчиков
